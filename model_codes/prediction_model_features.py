...
for grade in grades:
    ...
    for teacher in teachers:
        ...
        for i in range(len(sessions)):
            ...
            # print(prev_st_math_days)
            # 6. class size
            class_size = len(grade_session_teacher_df.student_entity_id.unique())
            # print(class_size)
            # 7. variance in start times
            start_time_variance = 0
            start_times.sort_values()
            mid_start_time = None
            ...
            if len(start_times) == 1:
                ...
                if len(start_times)%2 == 1:
                    ...
                    diff = ((start_times[half_way]-start_times[half_way-1]).total_seconds())/2
                    # print(start_times[half_way])
                    # print(start_times[half_way-1])
                    # print(diff)
                    mid_start_time = start_times[half_way-1]+datetime.timedelta(0,diff)
                X = 0
                # print(mid_start_time)
                for time_ in start_times.iteritems():
                    time = time_[1]
                    time_diff = 0
                    # print(time_[1])
                    # print(mid_start_time)
                ...
                start_time_variance = X/len(start_times)
            # print("start time variance:")
            # print(start_time_variance)


            # 8. variance in finish times
            finish_time_variance = 0
            finish_times.sort_values()
            ...
            if len(finish_times) == 1:
                ...
                if len(finish_times)%2 == 1:
                    ...
                    mid_finish_time = finish_times[half_way-1]+datetime.timedelta(0,diff)
                X = 0
                for time_ in finish_times.iteritems():
                    time = time_[1]
                    # print(time)
                    # print(mid_finish_time)
                    time_diff = 0
                    ...
                    X = X + time_diff*time_diff
                finish_time_variance = X/len(finish_times)
            # print(finish_time_variance)
            ...
            # print(disjointedness)     
            # end analyzing simultaneous plays
            # 10. class duration
            class_length = (finish_times.max() - start_times.min()).total_seconds()
            # print(class_length)
            ...
            if i > 0:
                    ...
                    time_difference = (start_times.min()-prev_session_data.python_time.max()).total_seconds()
            else:
                same_day_prev_session = 2 # first session conducted by a teacher in a grade
                common_students = 0
                time_difference = 0
            # print(same_day_session)
            ...
            data = [[district,year,iid,gcd,teacher_entity_id,session,date,session_start,session_finish,num_week_session,total_students,num_students_grade,num_prev_sessions,prev_st_math_days,class_size,start_time_variance,finish_time_variance,disjointedness,class_length,gameplay_duration,num_level_attempts,same_day_prev_session,common_students,time_difference,max_participent,performance,perf_rank,lab_seating,free_seating,segmented_class]]
