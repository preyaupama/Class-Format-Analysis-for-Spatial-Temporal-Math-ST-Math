        ...
        for teacher in teachers:
            ...
            for session_day in session_days:
                ...
                for row in session_day_data.itertuples():
                    ...
                    # first row is first session   
                    if row_number == 0:
                        session_num_laws[row.X1-1] = session_number
                    else:
                        # next level attempt within 10 minutes - same session - otherwise different session
                        time_difference = (row.python_time - prev_row.python_time).total_seconds()
                        if time_difference > 600:
                            session_number = session_number + 1
                        session_num_laws[row.X1-1] = session_number
                    #######################################################################
                    # whenever a new session is found new entry is created
                    ...
                    if all_session_info.get(session_number) is None:
                        if prev_row is not None: # complete previous session entry by adding finish time
                            all_session_info[session_num_laws[prev_row.X1-1]]['finish_time'] = prev_row.python_time
                    ...
                    if row.student_entity_id not in all_session_info[session_number]['students']: # append student to the current session
                        all_session_info[session_number]['students'].append(row.student_entity_id)
                    if row_number == len(session_day_data)-1: # last row of that session day
                ...
                for i in range(len(session_list)):
                    if i != len(session_list) - 1:
                        num_common_students = len(set(all_session_info[session_list_copy[i]]['students']).intersection(set(all_session_info[session_list[i+1]]['students']))) # number of common students with next session
                        time_gap = (all_session_info[session_list[i+1]]['start_time'] - all_session_info[session_list_copy[i]]['finish_time']).total_seconds() # time gap between current and next session
                        time_span = (all_session_info[session_list[i+1]]['finish_time'] - all_session_info[session_list[i]]['start_time']).total_seconds() # time span starting from start time of current session and finish time of next session
                        if time_span <= 2700:
                            all_session_info[session_list[i+1]]['students'].extend(all_session_info[session_list[i]]['students']) # add students
                            all_session_info[session_list[i+1]]['students'] = list(set(all_session_info[session_list[i+1]]['students'])) # remove duplicate students
                            session_list[i+1] = session_list[i] # update session number
                            revised = 1
                        else:
                            if num_common_students != 0:
                                data = [[num_common_students,all_session_info[session_list[i+1]]['students'],num_common_students/len(all_session_info[session_list[i+1]]['students']),time_gap]]
                                result_df = pandas.DataFrame(data)
                                result_df.to_csv("tune_merge_time_gap.csv",mode='a',header=None,index=False)
                            # if there are common students and time gap reasonable merge
                            if num_common_students/len(all_session_info[session_list[i+1]]['students']) >= 0.5 and time_gap <= 900:
                ...
                if revised == 1:
                    for row in session_day_data.itertuples():
                        session_num_laws[row.X1-1] = session_list[session_list_copy.index(session_num_laws[row.X1-1])] 
                ###########################################################################
                session_number = session_number + 1 # increase session number
                day = day + 1
            
            
df['session_num_laws'] = session_num_laws
